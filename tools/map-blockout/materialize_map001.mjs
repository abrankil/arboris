...
export function validateMap001Blockout(model) {
  ...
  assert(model.schemaVersion === '0.1', 'schemaVersion must be 0.1');
  assert(model.id === 'MAP-001-BLOCKOUT-CANDIDATE-001', 'unexpected blockout id');
  ...
  assert(model.interactionLearning?.status === 'OPEN', 'interaction/learning must remain OPEN');
  assert(Array.isArray(model.interactionLearning?.slots) && model.interactionLearning.slots.length === 0, 'interaction slots must not be invented');
