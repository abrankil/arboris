...
function contentCompatibility(a, b, profileA, profileB, traversal) {
  if (a === 'mixed' || b === 'mixed') {
    return { compatible: false, reason: 'edgeContent unresolved mixed' };
  }
  if (a === b) return { compatible: true, reason: null };
  const landWater = (a === 'land' && b === 'water') || (a === 'water' && b === 'land');
  if (landWater && profileA === 'bank' && profileB === 'bank' && traversal === 'closed') {
    return { compatible: true, reason: null };
  }
  return { compatible: false, reason: 'edgeContent mismatch' };
}
...
export function validateEdgeMatch(tileA, dirA, tileB) {
  ...
  const content = contentCompatibility(a.content, b.content, a.profile, b.profile, a.traversal);
  if (!content.compatible) errors.push(content.reason);
...
export function validatePatch(grammar) {
  invariant(grammar?.schemaVersion === '0.2', 'schemaVersion must be 0.2');
  invariant(grammar?.grammarId === 'TILE-GRAMMAR-CANDIDATE-v0.2', 'unexpected grammarId');
  ...
  invariant(grammar?.policies?.mixedEdge === 'UNRESOLVED_NON_CONNECTABLE', 'mixed edge policy must be UNRESOLVED_NON_CONNECTABLE');
...
export const TILE_GRAMMAR_CONSTANTS = {
  DIRECTIONS,
  OPPOSITE,
  MIXED_EDGE_POLICY: 'UNRESOLVED_NON_CONNECTABLE',
};
