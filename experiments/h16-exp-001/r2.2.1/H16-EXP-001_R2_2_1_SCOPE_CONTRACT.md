# H16-EXP-001 — R2.2.1 Scope Contract

**Estado:** DRAFT / CANDIDATE FOR VALIDATION  
**Versión:** R2.2.1  
**Ámbito:** Batch Annotation Workbench de H16-EXP-001  
**Base verificada:** main @ 035ff6edefd16df86eff5bc549efe7d41eae153e  
...
R2.2.1 preserva dos necesidades distintas dentro de una sola herramienta:
...
R2.2.1 separa autoridad botánica de autoridad operacional.
...
Para el dominio TRUSTED, APC es la autoridad operacional vigente. R2.2.1 no crea un segundo modelo normativo de persistencia de evidencia. La autoridad APC integrada en `main@035ff6edefd16df86eff5bc549efe7d41eae153e` incluye I1–I11 R4 y el contrato `ARBORIS_APC_I12_UI_CONTRACT_R10` VALIDATED WITH ASC / FROZEN. R2.2.1 consume esa autoridad para decisiones de implementación TRUSTED —incluidos UI, SHA-256, dedup, relink, revisiones y export— sin redefinir sus invariantes ni convertir el perfil BLIND en APC.
...
R2.2.1 puede reutilizar principios y capacidades demostradas por R2.1.3:
...
R2.2.1 no hereda automáticamente:
...
Las decisiones locales de R2.2.1 no reescriben retroactivamente R2.1.3.
...
La corrección debe conservar el historial y provenance exigidos por APC. R2.2.1 no redefine esos invariantes ni introduce un identificador alternativo de lineage.
...
R2.2.1 no introduce automatic ACE promotion.
...
Cualquier handoff TRUSTED → ACE debe utilizar el contrato APC/H16 vigente. R2.2.1 no crea un adaptador, shortcut ni equivalencia adicional.
...
La regla es normativa para R2.2.1. La cantidad o lista resultante es derivada de la autoridad vigente y no debe hardcodearse como una segunda regla.
...
Solo DIRECT_PHOTO_ELIGIBLE puede producir un commit de APC_EVIDENCE derivado de fotografía en el alcance base de R2.2.1, sujeto al contrato APC vigente.
...
R2.2.1 no ejecuta automáticamente aplica_si y no debe representar ninguno de estos casos retenidos mediante NOT_OBSERVABLE.

---

## 6. Elegibilidad, aplicabilidad y observabilidad

R2.2.1 conserva esta distinción:
...
R2.2.1 no debe inferir automáticamente aplicabilidad desde organ, view, filename, species_id o apariencia visual sin un contrato posterior que lo autorice.
...
## 7. Estados de observación R2.2.1

R2.2.1 admite exactamente:
...
salvo que evidencia procedimental específica permita establecer otra cosa. R2.2.1 no afirma que el anotador humano sea genéricamente “blind” en sentido total.
...
El scope R2.2.1 no congela una versión específica de photos.json. Cada workset o trusted binding debe registrar la identidad de la versión de inventario que realmente consumió.

---

## 13. Runtime authority binding

El freeze de R2.2.1 se validará contra versiones exactas de sus fuentes semánticas, pero una futura ejecución debe verificar la autoridad que realmente consume.
...
Esta semántica pertenece al ledger de ground truth del benchmark R2.2.1 y no constituye por sí sola una resolución general de evidencia repetida o contradictoria fuera de ese ledger.
...
R2.2.1 preserva explícitamente:
...
El modelo operacional usado por R2.2.1 no constituye cierre de OPEN-02.

La distinción entre elegibilidad y aplicabilidad no introduce reglas por view y no cierra OPEN-03.

---

## 18. Decisiones R2.2.1 diferidas
